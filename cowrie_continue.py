...
Original file is located at
    https://colab.research.google.com/drive/1U_QpvVoxdY-BxdK0lSe7JjTnAyHsxKUO
"""
...
class CowrieAnalyzer:
    def __init__(self, json_dir='log'):
        self.files = glob.glob(json_dir + os.sep + 'cowrieData.jso*')
        self.num_ssh, self.num_telnet = 0, 0
        self.src_ip_cnt = defaultdict(int) 
    ...
    def plot(self):
        ...
        ssh_attempts = zip(*sorted(self.ssh_times_cnt.items()))
        #ax.plot(dates, ssh_attempts, 'b-', label='SSH Attempts')
        telnet_attempts = zip(*sorted(self.telnet_times_cnt.items()))
        ...
        for file in self.files:
            with open(file) as openfile:
                for line in openfile:
                    total_contents.append(ast.literal_eval(line))

        num_ssh, num_telnet = 0, 0
        for event in (total_contents):
            if 'cowrie.login' in event[0]['eventid']:
